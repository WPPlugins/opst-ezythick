...
function embed_thickbox( $atts, $content = null ) {
  ...
  switch ($a['type']) {
    case 'iframe' : return ezyThick_Div($a, $content, 'iframe');
      break;
    default:  return ezyThick_Div($a, $content, 'inline');
  }
}
/**
 ...
 */
function ezyThick_Div($a, $content, $type) {

// Create the link
  $tpl = "<a class='thickbox' href='".$a['src']."#TB_".$type."?inlineId=".$a['inlineid'].
...
// Add "close"-button in modal mode only
